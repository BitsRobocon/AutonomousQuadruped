...
class ARSAgent():
    def __init__(self,
        ...
        self.spot = spot
        if gui:
            self.g_u_i = GUI(self.env.vrka.quadruped)
        else:
            self.g_u_i = None
    ...
    def deployTG(self, direction=None, delta=None):
        ...
        while not done and timesteps < self.policy.episode_steps:
            ...
            self.env.vrka.GetExternalObservations(self.TGP, self.smach)
    ...
    def save(self, filename):
        ...
        with open(filename + '_policy', 'wb') as filehandle:
            pickle.dump(self.policy.theta, filehandle,encoding = 'bytes')
